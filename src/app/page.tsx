...
export default function Home() {
  return (
    <>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center mt-20">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link 
                href="/events" 
                className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-bmki-gold text-bmki-purple font-bold rounded-full hover:bg-white hover:scale-105 transition-all flex items-center justify-center gap-2"
              >
                Get Season 6 Tickets <ArrowRight size={18} />
              </Link>
              <Link 
                href="/talent-portal" 
                className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-transparent border-2 border-white text-white font-bold rounded-full hover:bg-white hover:text-bmki-purple transition-all"
              >
      ...
      {/* Countdown Section */}
      <section className="py-12 sm:py-16 bg-bmki-purple relative overflow-hidden">
        <div className="absolute inset-0 opacity-10 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-white to-transparent"></div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-8 sm:mb-12">
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-4">Season 6 is Coming</h2>
            <p className="text-bmki-gold text-lg sm:text-xl">The biggest creative showcase in ABU Zaria</p>
          </div>
      ...
      {/* Features Highlights */}
      <section className="py-12 sm:py-16 bg-neutral-50 dark:bg-neutral-950">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12 sm:mb-16">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4 dark:text-white">What We Do</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
            {[
              ...
              <motion.div 
                key={idx}
                className="bg-white dark:bg-neutral-900 p-6 sm:p-8 rounded-2xl shadow-lg border border-gray-100 dark:border-neutral-800 hover:border-bmki-gold/50 transition-colors group"
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: idx * 0.2 }}
              >
                <div className="w-14 h-14 bg-bmki-gold/20 rounded-full flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                  {feature.icon}
                </div>
                <h3 className="text-xl sm:text-2xl font-bold mb-3 dark:text-white">{feature.title}</h3>
