...
export default function ServicesPage() {
  return (
    <div className="pt-20 bg-neutral-50 dark:bg-neutral-950 min-h-screen">
      ...
      <section className="pb-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {servicesList.map((service, idx) => (
            <motion.div
              key={idx}
              className="bg-white dark:bg-neutral-900 p-8 rounded-3xl shadow-sm hover:shadow-xl border border-gray-100 dark:border-neutral-800 transition-all group"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: idx * 0.1 }}
            >
              <div className="w-16 h-16 bg-bmki-purple/5 text-bmki-purple dark:bg-bmki-gold/10 dark:text-bmki-gold rounded-2xl flex items-center justify-center mb-6 group-hover:bg-bmki-purple group-hover:text-white dark:group-hover:bg-bmki-gold dark:group-hover:text-bmki-purple transition-colors">
                {service.icon}
              </div>
      ...
      {/* CTA Section */}
      <section className="py-20 bg-bmki-purple text-center px-4">
        <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">Need a custom creative solution?</h2>
